    ...
    courseModel.findOne({ '_id': req.session.courseId }, (err, user) => {
        ...
        educatorModel.findOne({ 'educatorEmail': req.session.email }, (err, user) => {
            if (err)
                ...
                res.render('educator/course_landing_page', { 'isdoneclp': isdoneclp, 'category': category, 'title': title, 'des': des, 'level': level, 'prim': prim, 'lang': lang, 'cid': cid, 'name': name, 'isDP': isDP, 'about': about, 'email': req.session.email })
            }
        })
    })
})
        ...
        if (err) console.log(err)
            ...
            educatorModel.findOne({ 'educatorEmail': req.session.email }, (err, data) => {
                if (err) console.log(err)
                    ...
                    var status = ''
                    if (data['courses'][user['courseCategory']][req.session.courseId]['courseStatus']) {
                        status = data['courses'][user['courseCategory']][req.session.courseId]['courseStatus']
                    }
...
router.get('/download/course-annexure-1/:cat/:cid', (req, res) => {
    ...
    var path = __basedir + "/uploads/courseVideos/"+cat+"/"+req.params.cid+"/course-annexure-1.xlsx";
    res.download(path)
})
...
router.post('/upload/testDemoVideo', uploadTV.single("test_video"), (req, res) => {
    ...
    var newValues = { $set: { testVideo: req.file.originalname, isVerified: 1 } };
    educatorModel.updateOne({ 'educatorEmail': req.session.email }, newValues, (err, user) => {
        if (err) {
            res.send(err)
        } else {
...
router.post('/upload/annexure', uploadAX.single("annexure"), (req, res) => {
    if (!req.session.email) {
        ...
        xlsxFile(path).then((rows) => {
            newCourse.courseOverview = rows[1][2]
            newCourse.targetAudience = rows[2][2]
            newCourse.courseType = rows[3][2]
            newCourse.courseNature = rows[4][2]
            newCourse.courseObjective = rows[5][2]
            newCourse.courseDescription = rows[6][2]
            newCourse.courseCurriculum = rows[7][2]
            newCourse.courseIntroVideo = rows[8][2]
            newCourse.coursePrereq = rows[9][2]
            newCourse.intendedAudience = rows[10][2]
            newCourse.courseDuration = rows[11][2]
            newCourse.learningOutcomes = [rows[12][2], rows[13][2], rows[14][2], rows[15][2], rows[16][2], rows[17][2], rows[18][2]]
            newCourse.assessmentPlan = rows[19][2]
            newCourse.courseUploadDate = new Date()
                ...
                if (err) {
                    ...
                    courseModel.updateOne({ '_id': req.session.courseId }, newValues, (err, user) => {
                        if (err) {
                            ...
                            console.log(user)
                            var courses = {}
                            var courseKey = 'courses.' + ccategory + '.' + req.session.courseId + '.title'
                            ...
                            educatorModel.updateOne({ 'educatorEmail': req.session.email }, newVal, (err, user) => {
                                if (err) {
                                    ...
                                    if (user == null || user['ok'] == 0) {
                                        ...
                                        var newpath = __basedir + '/uploads/courseVideos/'+req.session.category+'/'+req.session.courseId
                                        try {
...
router.post('/delete/additional/:week', (req, res) => {
    ...
    if(req.body.type == 'file') {
        // var path = __basedir + "/uploads/courseVideos/" + req.session.category + "/" + req.session.courseId + "/" + req.params.week
        // fs.unlink(path, (err) => {
        //     if (err) console.log(err);
        //     else {
        //         console.log('del')
...
router.post('/save/courseLandingPage', (req, res) => {
    console.log(req.session.courseId)
    console.log(req.body)
    console.log(req.files)
        // level: req.body.level
    var newValues = { $set: { clpage: 'Y', courseDescription: req.body.des, primarilyTaught: req.body.primarily_taught, level: req.body.level } };
    courseModel.updateOne({ '_id': req.session.courseId }, newValues, (err, user) => {
        if (err) {
            console.log(err)
            res.send(err)
...
router.post('/update/educatorProfile', uploadDP.single('profile_photo'), (req, res) => {
    ...
    if (req.file)
        ...
        var dp = 'N'
    var newValues = { $set: { about: req.body.instructor_des, dp: dp } };
    educatorModel.updateOne({ 'educatorEmail': req.session.email }, newValues, (err, user) => {
        if (err) {
            console.log(err)
            res.send(err)
...
router.post('/save/pricing', (req, res) => {
    ...
    courseModel.findOneAndUpdate({ '_id': req.session.courseId }, newVal, (err, user) => {
        if (err) res.send('Error while saving')
            ...
            var newK = {}
            var key = 'courses.' + user['courseCategory'] + '.' + req.session.courseId + '.pricing'
...
router.get('/submitforreview', (req, res) => {
    courseModel.findOne({ '_id': req.session.courseId }, (err, user) => {
        if (err)
            ...
            console.log(flag)
            if (flag == 1)
                res.send('Please upload the video lectures for all the weeks first.')
                ...
                educatorModel.findOne({ 'educatorEmail': req.session.email }, { 'dp': 1, 'about': 1 }, (err, resp) => {
                    if (err) res.send('Something went wrong. Try after some time.')
                        ...
                        if (!(resp['dp'] != 'N' && resp['about'] && resp['about'].length > 50))
                            ...
                            var newK = {}
                            var key = 'courses.' + user['courseCategory'] + '.' + req.session.courseId + '.courseStatus'
                            newK[key] = 'Review'
                            var newVal = { $set: newK }
...
router.post('/save/mou', (req, res) => {
    ...
    console.log(msg)
    var newVal = { $set: { mouDetails: { 'basic': msg, 'file': 'sig.jpg', 'witness1': { 'name': req.body.w1_name, 'address': req.body.w1_add, 'file': 'sig.jpg' }, 'witness2': { 'name': req.body.w2_name, 'address': req.body.w2_add, 'file': 'sig.jpg' } } } }
    courseModel.updateOne({ '_id': req.session.courseId }, newVal, (err, user) => {
        if (err) {
            console.log(err)
